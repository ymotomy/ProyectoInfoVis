const WIDTH = 900,
  HEIGHT = WIDTH * 0.6;
const margin = { top: 70, bottom: 70, left: 100, right: 50 };
...
const generarGrafico = (data, hemisphere) => {
  const apilador = d3.stack().keys(["Fish", "Insects", "Sea Creatures"]);
  ...
  d3.selectAll(".bullet-sea-creatures").style(
    "color",
    escalaColor("Sea Creatures")
  );
  const area = d3
    .area()
  ...
  function mouseMove(event) {
    ...
    tooltip
      .select("#sea-creatures")
      .html(closestDataPoint.data["Sea Creatures"]);
